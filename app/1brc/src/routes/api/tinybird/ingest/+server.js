...
export async function GET({ url }) {
    let result = await fetch(
        'https://api.tinybird.co/v0/events?name=streaming_measurements',
        {
            method: 'POST',
            body: generate(),
            headers: { Authorization: `Bearer ${TINYBIRD_API_KEY}` }
        }
    )
        .then(res => res.status)

    if (result != 202) {
        console.error(`Failed to ingest, response ${result}`);
    } else {
        return json(result);
    }
}
